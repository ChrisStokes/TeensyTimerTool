...
namespace TeensyTimerTool
{
    ...
    class TMRChain_t : public TMR_t<moduleNr>
    {
     public:
        static ITimerChannel* getTimer32();
        static ITimerChannel* getTimer64();
     ...
     protected:
        ...
        // bool TMR32_t<moduleNr>::isInitialized = false;

        // template <unsigned m>
        // callback_t TMR32_t<m>::callbacks[4];
    };

    // IMPLEMENTATION ==================================================================

    template <unsigned moduleNr>
    IMXRT_TMR_t* const TMRChain_t<moduleNr>::pTMR = moduleNr == 0 ? &IMXRT_TMR1 : moduleNr == 1 ? &IMXRT_TMR2 : moduleNr == 2 ? &IMXRT_TMR3 : &IMXRT_TMR4;
    template <unsigned moduleNr>
    IMXRT_TMR_CH_t* const TMRChain_t<moduleNr>::pCH0 = &pTMR->CH[0];
    template <unsigned moduleNr>
    IMXRT_TMR_CH_t* const TMRChain_t<moduleNr>::pCH1 = &pTMR->CH[1];
    template <unsigned moduleNr>
    IMXRT_TMR_CH_t* const TMRChain_t<moduleNr>::pCH2 = &pTMR->CH[2];
    template <unsigned moduleNr>
    IMXRT_TMR_CH_t* const TMRChain_t<moduleNr>::pCH3 = &pTMR->CH[3];

    template <unsigned moduleNr>
    ITimerChannel* TMRChain_t<moduleNr>::getTimer32()
    {
        auto t1 = TMR_t<moduleNr>::getTimer16();
        auto t2 = TMR_t<moduleNr>::getTimer16();
    ...
    // template <unsigned m>
    // bool TMR32_t<m>::isInitialized = false;
